...
- [4. 手感优化方案·更少的Z和X·有拆分提示·征求意见稿](#4-手感优化方案更少的z和x有拆分提示征求意见稿)
  ...
  - [4.3. 重码數據](#43-重码數據)
  - [4.4. 手感數據](#44-手感數據)
    - [4.4.1. 简体文本调整版数据](#441-简体文本调整版数据)
    - [4.4.2. 简体文本原码表数据](#442-简体文本原码表数据)
    - [4.4.3. 繁体文本调整版数据](#443-繁体文本调整版数据)
  - [4.5. 改动](#45-改动)
  - [4.6. 方案名](#46-方案名)
  - [4.7. 字根圖](#47-字根圖)
...
- 改动字根时不过分影响记忆。小码尽量使用拼音中的字母帮助记忆(若无法达到,使用中间一行的`asdfghjkl`九键)。一般的，借用`k`代替`ch`和`k`入聲尾，借用`j`代替`i`，借用`s`代替`z`或`x`。
...
- GB2312重碼：320 降至 275
- 常用8000繁簡重碼：367 降至 317
- GBK重碼：2883 降至 2769
- 全字集**大約**（因爲我還有四千箇字没拆完）重碼：43019 降至 41985
...
- 原徐碼獨有的重碼字爲：媾缅 缝终 酞裁 掭威 捌拐 轨 恶蕲 雩需 此柴 踊嗵 铺镖 铒钳 钨钓 钤铂 急怨 鲠詹 馑饵 鱿鲒 舱舶 徵微徽 仪俯 逾怔 鼯鸽 朗禄 寥礼 裰初 剪前 部亨 漏记 诵涌 漤涛 鹇闫 溻谒 溃闾 澹讽

- 新方案獨有的重碼字爲：袈迦 襞避 拆逝 鸷热 裘逑 钛链 镶锎锏 舔适 鹧庹 庶遮 鹑烹 注润
...
- **「九」由`Rj`改为`Rh`。降重，把二簡讓給「近」。**
- **「戶」由`Rh`改为`Rf`。**
- **「瓜」由`Rg`改为`Rk`。**
- **「彳」由`Sr`改为`Sm`。減少「徊徜膛」重碼**
- **「乌」由`Sw`改为`Sl`。**
