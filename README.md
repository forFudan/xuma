...
- 大量减少 Z 键和 X 键的使用频率。
- 在 GB2312 和 GBK 字集中，降低重码。
- 改动字根时不过分影响记忆。小碼使用拼音幫助記憶。

基于此，我对徐码的字根码位进行了以下改动。大小码都改动的有：
...
- 增加「微」字头（卽`彳山攵`），碼位为`Su`。

改动后，GB2312字集下重码字数由320降低到了317，GBK字集下重码字数由2883降低到了2864。

本方案名为`xuma2022forfudan_ergo`。
