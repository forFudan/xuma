...
特别的，部分字频不高的繁体字和传承字字根移到二选或者三选。这样可以腾出一个二简码位用于高频字。例如：
...
- 「缶0.00%」移到二选，同时将码位让给「年0.18%」
- 「臣0.01%」移到二选，同时将码位让给「打0.15%」
- 「瓜0.01%」移到二选，同时将码位让给「后0.34%」
- 「虫0.01%」移到二选，同时将码位让给「些0.20%」
- 「申0.00%」移到二选，同时将码位让给「明0.15%」
- 「乌0.01%」移到二选，同时将码位让给「往0.06%」
- 「烏0.01%」移到二选，同时将码位让给「今0.07%」
- 「牛0.02%」移到二选，同时将码位让给「算0.07%」
- 「馬0.09%」移到二选，同时将码位让给「现0.22%」
- 「弋0.00%」移到二选，同时将码位让给「式0.03%」
- 「广0.02%」移到二选，同时将码位让给「定0.14%」
- 「匕0.00%」移到二选，同时将码位让给「条0.05%」
- 「骨0.02%」移到二选，同时将码位让给「光0.12%」
