...
- [4. 手感优化方案（更少的Z和X）](#4-手感优化方案更少的z和x)
  - [4.1. 动机](#41-动机)
  - [4.2. 改动](#42-改动)
  - [4.3. 重码](#43-重码)
  - [4.4. 按鍵頻率](#44-按鍵頻率)
...
徐码中，不少常用字根分布在 Z 键和 X 键 上，导致此两键的频率过高，这个情况在打简体时尤甚，对左手的小指造成了很大的负担。因此，我对徐码的方案进行了调整，以达到以下目的。

- 大量减少 Z 键和 X 键的使用频率。
- 在 GB2312 和 GBK 字集中，降低静态重码和动态重码(字频加权)。
- 改动字根时不过分影响记忆。小码尽量使用拼音中的字母帮助记忆(若无法达到,则就近使用中间一排字母)。

基于此，我对徐码的字根码位进行了以下改动。

### 4.2. 改动

首先，将Z键上的副根全部移走。X键上只留下「米」「羊」「冫」三根（我实在移不走了），改动的有：

1. 「讠」由`Zy`改为`Wn`。解放小拇指。
2. 「辶」由`Zd`改为`Wk`。解放小拇指。
3. 「之」由`Zz`改为`Wh`。解放小拇指。
4. 「户」由`Zh`改为`Wf`。解放小拇指。
5. 「门」由`Zm`改为`Wm`。解放小拇指。
6. 「灬」和「四点」由`Xh`改为`Sh`。解放无名指。
7. 「為」由`Xw`改为`Ww`。解放无名指。

其次，將小码爲x和z的字根全部進行調整，解放小拇指的有：

- 「子」由`Bz`改为`Ba`。解放小拇指，就近选取字母`a`。
- 「長」由`Ez`改为`Ea`。解放小拇指。
- 「至」由`Fz`改为`Fh`。解放小拇指。
- 「走」由`Jz`改为`Js`。解放小拇指。
- 「早」由`Kz`改为`Ka`。解放小拇指。
...
- 「耳」由`He`改为`Hr`。降重。
- 「丌」由`Hj`改为`Hi`。降重。
- 「贝」由`Mb`改为`Mi`。提升手感，降重。
- 「貝」由`Nb`改为`Ni`。提升手感，降重。
- 「〇」由`Ol`改为`Oo`。它就是个圈。
- 「白」由`Ub`改为`Ui`。降重
- 「鼠」由`Us`改为`Uh`。降重
- 「礻」由`Ws`改为`Wd`。降重
...
字母按键频率按照从多到少排序为:
`uigvwosbekfjtadcplryhqnmxz`

字母按键频率按照从少到多排序为:
`zxmnqhyrlpcdatjfkebsowvgiu`
