...
- [2. 字根編碼](#2-字根編碼)
  - [2.1. 大碼和小碼](#21-大碼和小碼)
  - [2.2. 字根圖](#22-字根圖)
  - [2.3. 字根大碼的分佈規律](#23-字根大碼的分佈規律)
  - [2.4. 字根小碼的分佈規律](#24-字根小碼的分佈規律)
- [3. 拆字規則](#3-拆字規則)
  - [3.1. 字根最少](#31-字根最少)
...
![徐碼字根圖](/resources/徐碼字根鍵位圖_20220411_新徐.jpg)
...
我剛剛用的「部」字爲例，說明了拆字的一般形式。這種將字拆成直觀的、分離的部件的方法，足以應付大部分漢字的拆分了。但在實際使用中，遇到這個問題：一個獨體字，本身不是字根，怎麼辦？這時候，我們要把它繼續拆成更小的字根甚至筆劃。比如：
...
注意，**原形字根和變形字根都一定存在於字根圖中**。字根圖沒有的字根永遠不能叫做變形字根。
...
- 「嫩」字拆成```女Cv 木Iv 口Ov 攵pp```四根。分別對應了首根、次根、次末根、最末根。我們直接取四個字根的大碼```CIOP```即可出字。
...
- 「织」字拆成```纟Cj 口Ov 八Tu```三根。分別對應了首根、次根、最末根。我們取三個字根的大碼```COT```，再添上首根的小碼```j```作識別。輸入```COTj```即可出字。
...
- 一個字的編碼等於4位，後面沒有其他的字需要輸入，則需要按空格鍵上屏首選。
...
二級簡碼可見以下矩陣。其中，第一列的字母爲第一碼，第一排的字母爲第二碼。
