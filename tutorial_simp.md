...
<https://zhuyuhao.com/xuma/tutorial_simp>
...
我刚刚用的「部」字为例，说明了拆字的一般形式。这种将字拆成直观的、分离的部件的方法，足以应付大部分汉字的拆分了。但在实际使用中，依旧会遇到两个特殊情况：
...
徐码，通过一套严密的规则，使每一个汉字，都只有**唯一**的拆分方案，只有唯一的编码，完全消除了不确定性。
...
字根首笔符合笔顺，意思是拆分一个汉字时，应该让所有字根的首个笔划，符合这个汉字的笔顺。这个规则是用来排列字根顺序的。比如：
...
在徐码的字根中，有一部分字根是由其他的字根变形而来的。字根图中，这部分**变形根**列在字根原型的右下方，以小字显示。比如：
